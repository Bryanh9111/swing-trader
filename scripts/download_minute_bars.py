#!/usr/bin/env python3
"""Download 1-minute bars from Polygon REST API for all symbols in by_symbol cache.

Saves to: .cache/historical_minutes/{SYMBOL}.csv.gz

Usage:
    python scripts/download_minute_bars.py --start 2026-01-01 --end 2026-01-31
    python scripts/download_minute_bars.py --start 2026-01-01 --end 2026-01-31 --workers 10
"""
...
def _parse_args(argv: list[str] | None = None) -> DownloadArgs:
    parser = argparse.ArgumentParser(description="Download 1-minute bars from Polygon.")
    ...
    parser.add_argument("--output-dir", default=None, help="Output directory. Default: .cache/historical_minutes/{start}_{end}")
...
def _fetch_minute_bars(symbol: str, start: date, end: date, api_key: str) -> list[dict[str, Any]]:
    """Fetch all 1-minute bars for a symbol via Polygon REST API with pagination."""
...
def _download_one(
    ...
    """Download minute bars for one symbol. Returns (symbol, bar_count, status)."""
    ...
    try:
        bars = _fetch_minute_bars(symbol, start, end, api_key)
        if not bars:
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.output_dir:
        ...
        output_dir = project_root / ".cache" / "historical_minutes" / f"{args.start}_{args.end}"
    ...
    print(f"Downloading 1-min bars: {len(symbols)} symbols")
    ...
    print()
    print("=" * 60)
    print("Minute Bar Download Summary")
    print("=" * 60)
