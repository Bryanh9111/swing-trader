#!/usr/bin/env python3
"""Download 1-hour bars from Polygon REST API for all symbols in by_symbol cache.

Saves to: .cache/historical_hours/{start}_{end}/{SYMBOL}.csv.gz

Usage:
    python scripts/download_hour_bars.py --start 2025-01-01 --end 2025-01-31
    python scripts/download_hour_bars.py --start 2025-01-01 --end 2025-12-31 --workers 3
"""
...
def _parse_args(argv: list[str] | None = None) -> DownloadArgs:
    parser = argparse.ArgumentParser(description="Download 1-hour bars from Polygon.")
    ...
    parser.add_argument("--output-dir", default=None, help="Output directory. Default: .cache/historical_hours/{start}_{end}")
    ns = parser.parse_args(argv)
    start = datetime.strptime(ns.start, "%Y-%m-%d").date()
    end = datetime.strptime(ns.end, "%Y-%m-%d").date()
    return DownloadArgs(start=start, end=end, workers=max(1, ns.workers), force=ns.force, output_dir=ns.output_dir)
...
def _fetch_hour_bars(symbol: str, start: date, end: date, api_key: str) -> list[dict[str, Any]]:
    """Fetch all 1-hour bars for a symbol via Polygon REST API with pagination."""
    all_results: list[dict[str, Any]] = []
    base_url = (
        f"https://api.polygon.io/v2/aggs/ticker/{symbol}/range/1/hour"
...
def _save_csv_gz(bars: list[dict[str, Any]], output_path: Path) -> int:
    """Save bars to compressed CSV. Returns row count."""
    if not bars:
        return 0
...
def _download_one(
    ...
    """Download hour bars for one symbol. Returns (symbol, bar_count, status)."""
    ...
    try:
        bars = _fetch_hour_bars(symbol, start, end, api_key)
        if not bars:
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.output_dir:
        ...
        output_dir = project_root / ".cache" / "historical_hours" / f"{args.start}_{args.end}"
    ...
    print(f"Downloading 1-hour bars: {len(symbols)} symbols")
    ...
    with ThreadPoolExecutor(max_workers=args.workers) as executor:
        ...
        for symbol in symbols:
            fut = executor.submit(
                _download_one, symbol, args.start, args.end, api_key, output_dir, args.force
            )
            futures[fut] = symbol
    ...
    print()
    print("=" * 60)
    print("Hour Bar Download Summary")
    print("=" * 60)
