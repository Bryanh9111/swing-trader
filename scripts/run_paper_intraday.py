#!/usr/bin/env python3
"""Run INTRADAY_CHECK in paper trading mode.

This script runs the full pipeline at 10:30 or 14:30 intraday slots.
Currently runs the same pipeline as PRE_MARKET_FULL_SCAN (full universe→scan→strategy→risk→execution).
Future optimization (M1): execution-only lightweight path.

Usage:
    # Dry run 10:30 slot
    python scripts/run_paper_intraday.py --slot 1030 --dry-run

    # Paper trading 14:30 slot
    python scripts/run_paper_intraday.py --slot 1430 --paper
"""
...
SLOT_MAP = {
    "1030": RunType.INTRADAY_CHECK_1030,
    "1230": RunType.INTRADAY_CHECK_1230,
    "1430": RunType.INTRADAY_CHECK_1430,
}


def main():
    parser = argparse.ArgumentParser(description="Run INTRADAY_CHECK in paper trading mode")
    parser.add_argument(
        "--slot",
        required=True,
        choices=["1030", "1230", "1430"],
        help="Intraday slot: 1030, 1230, or 1430",
    )
    ...
    run_type = SLOT_MAP[args.slot]
    print(f"\nRunning {run_type.value} in {mode.value} mode")
    ...
    if mode is OperatingMode.PAPER:
        ...
        if ibkr_ok:
            print(f"   ✅ IB Gateway connectivity verified")
        else:
            print(f"   ⚠️  IB Gateway unreachable — run will proceed in degraded mode")
