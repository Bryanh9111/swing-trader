#!/usr/bin/env python3
"""Run PRE_CLOSE_CLEANUP in paper trading mode.

This script runs the pre-close cleanup at 15:55 ET:
- Cancel open entry orders (keep protective stops)
- Verify / generate missing protective stops
- Reconcile positions

The orchestrator short-circuits to execution-only (skips
universe/scanner/strategy/risk_gate).

Usage:
    # Dry run
    python scripts/run_paper_pre_close_cleanup.py --dry-run

    # Paper trading
    python scripts/run_paper_pre_close_cleanup.py --paper
"""
...
def main():
    parser = argparse.ArgumentParser(description="Run PRE_CLOSE_CLEANUP in paper trading mode")
    ...
    print(f"\nRunning PRE_CLOSE_CLEANUP in {mode.value} mode")
    ...
    run_config["run_type"] = RunType.PRE_CLOSE_CLEANUP.value
